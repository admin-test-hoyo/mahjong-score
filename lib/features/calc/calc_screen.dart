  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Padding(padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom, left: 20, right: 20, top: 24), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(
          ...
          children: [
            const Text('役満賞', style: TextStyle(color: Colors.white54, fontSize: 11)),
            const SizedBox(width: 16),
            const Text('ツモ', style: TextStyle(color: Colors.white38, fontSize: 10)),
            const SizedBox(width: 4),
            SizedBox(width: 50, child: _field(ref, '', config.yakumanTsumoPrize.toString(), (v) => ref.read(configProvider.notifier).updateYakumanTsumoPrize(int.tryParse(v) ?? 5))),
            const SizedBox(width: 16),
            const Text('ロン', style: TextStyle(color: Colors.white38, fontSize: 10)),
            const SizedBox(width: 4),
            SizedBox(width: 50, child: _field(ref, '', config.yakumanRonPrize.toString(), (v) => ref.read(configProvider.notifier).updateYakumanRonPrize(int.tryParse(v) ?? 10))),
        ]),
        const SizedBox(height: 12),
